...
Creates a new `NodePatcher` instance for intercepting and transforming React component render output at the element level.
...
**Returns:** `NodePatcher` – A new patcher instance (see below).

___

# ReactUtils.NodePatcher

`NodePatcher` is the class returned by `BdApi.ReactUtils.createNodePatcher()`. It replaces a React element's `.type` with a wrapped version that passes the rendered output through a transform callback, allowing plugins to modify what a component renders.
...
- A `Symbol("BetterDiscord.NodePatcher")` marker to detect already-patched types.
- A `WeakMap` cache mapping original component types to their patched counterparts.
- A destroyed flag checked by all patched wrappers.

## Methods

### patch

Patches a React element's `.type` in-place so that, when rendered, the component's output is passed through `transform` before being returned. Handles class components, function components, and exotic wrappers (memo/forwardRef/lazy).

```ts
nodePatcher.patch<P>(
  element: { type: React.ComponentType<P> },
  transform: (props: P, res: React.ReactNode, instance?: React.Component<P>) => React.ReactNode
): void;
```
...
| Parameter |       Type       | Optional | Default |                       Description                        |
|:----------|:-----------------|:--------:|:-------:|:---------------------------------------------------------|
element|object|&#x274C;|*none*|A React element whose `.type` will be replaced in-place|
transform|function|&#x274C;|*none*|Callback invoked after the original component renders (see signature below)|

**Transform callback signature:**

| Argument |       Type       |                          Description                          |
|:---------|:-----------------|:--------------------------------------------------------------|
props|`P`|The props passed to the component|
res|`React.ReactNode`|The original rendered output of the component|
instance|`React.Component<P>` \| `undefined`|The class component instance (only provided for class components; `undefined` for function components)|

**Return value of `transform`:**

- Return a `React.ReactNode` to replace the original output.
- Return `undefined` to keep the original output unchanged.

**Behavior by component type:**

| Component type | How it patches |
|:---------------|:---------------|
Class component (`isReactComponent`)|Creates a subclass that overrides `render()`. Calls `super.render()`, then passes the result through `transform(this.props, res, this)`.|
Function component|Wraps the function. Calls the original, then passes the result through `transform(props, res)`. If the original returns a `Promise` (async component), the transform is applied inside `.then()`.|
`React.memo`|Unwraps to the inner type, patches it, then re-wraps with `React.memo` (preserving the original `compare` function).|
`React.forwardRef`|Unwraps to the inner `render`, patches it, then re-wraps with `React.forwardRef`.|
`React.lazy`|Wraps the lazy initializer so the resolved component is patched on first load.|
Nested wrappers (e.g. `memo(forwardRef(...))`)|Handled — `memo` wrapping a `forwardRef` is detected and re-wrapped correctly.|

**Caching:**

- Patched types are cached in an internal `WeakMap`. If `patch` is called again with the same original `.type`, the cached version is reused immediately.
- Patched types are also marked with an internal `Symbol`, providing a secondary fast-path lookup directly on the type object.
...
// Inside a Patcher.after on a parent component's render:
BdApi.Patcher.after("MyPlugin", ParentComponent.prototype, "render", (thisObj, args, ret) => {
    const targetElement = /* find target element in ret's children */;
    if (!targetElement) return;

    patcher.patch(targetElement, (props, res) => {
        // Modify the rendered output
        if (res?.props?.children) {
            res.props.children.push(myCustomElement);
        }
        // Return undefined to keep res as-is (after mutation),
        // or return a new ReactNode to replace it entirely.
    });
});
```
...
- Call `patch` on elements that are part of an active render tree (i.e., elements returned from a component's render). The method mutates `element.type` in-place, so React will render the patched version.
- Do not recreate a `NodePatcher` every render without calling `destroy()` — reuse a single instance per lifecycle scope.
- Copies `defaultProps`, `displayName`, and `propTypes` from the original type to the patched type for function components.

___

### destroy

Disables all patches created by this `NodePatcher` instance. Patched components immediately stop applying the transform and return their original output.

```ts
nodePatcher.destroy(): void;
```
...
- Sets an internal destroyed flag.
- All patched wrappers (both class and function) check this flag at render time. Once destroyed, they short-circuit and return the original rendered output without calling `transform`.
- This avoids the need to individually unwrap or restore each patched type — a single `destroy()` call cleanly disables everything.
- The `WeakMap` cache and patched types remain in memory but become no-ops. They will be garbage-collected once no references to the patched types exist.

**Usage example:**

```js
class MyPlugin {
    nodePatcher = BdApi.ReactUtils.createNodePatcher();

    start() {
        // ... set up patches using this.nodePatcher.patch(...)
    }
...
### Recommended pattern: Patcher.after + NodePatcher

The typical workflow for patching a deeply nested component:

1. Use `BdApi.Patcher.after` on a stable parent component's render method.
2. Traverse the returned React element tree to locate the target element.
3. Call `nodePatcher.patch(targetElement, transform)` on it.
4. On plugin stop, call both `BdApi.Patcher.unpatchAll` and `nodePatcher.destroy()`.
...
class MyPlugin {
    ...
    start() {
        BdApi.Patcher.after("MyPlugin", ParentComponent.prototype, "render", (thisObj, args, ret) => {
            const target = BdApi.Utils.findInTree(ret, e => e?.type?.displayName === "TargetComponent", {
                walkable: ["props", "children"]
            });
            if (!target) return;

            this.nodePatcher.patch(target, (props, res) => {
                // Transform the target component's output
                return res;
            });
        });
    }
...
**When to prefer `createNodePatcher` over `Patcher.after` on render:**

- You only have access to an element in a render tree, not the module/class directly.
- The target component may be wrapped in `memo`, `forwardRef`, or `lazy` — `NodePatcher` handles unwrapping automatically.
- You want a single `destroy()` cleanup for all element-level patches.
